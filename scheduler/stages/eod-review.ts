...
export async function runEodReview(db: SupabaseClient, runDate: string) {
  ...
  const useAlpaca = brokers.length > 0;
  logs.push(
    useAlpaca
      ? `End-of-day review — closing on ${brokers.map((b) => b.mode).join(" + ")} (live trading=${liveTradingEnabled()})`
      : `End-of-day review — database-only mode`
  );
  ...
  // ── Close positions on every active broker ──
  for (const broker of brokers) {
    try {
      const closed = await broker.closeAllPositions();
      logs.push(`${broker.mode}: ${closed.length} position(s) close order(s) submitted`);
    } catch (err) {
      logs.push(`${broker.mode} close error: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  if (paperClient.available) {
    try {
      ...
      logs.push(`paper account: $${finalValue.toFixed(2)} portfolio, $${finalCash.toFixed(2)} cash`);
    } catch {
